...
namespace UI_GestionRRHH.Formularios.Personal
{
    public partial class frmCargaMasiva : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        string cod_pais = "", cod_departamento = "", cod_provincia = "", cod_distrito = "", dsc_departamento = "", dsc_provincia = "", dsc_distrito = "", cod_empresa = "", cod_tipo_via = "",cod_trabajador="";
                    ...
                    if (frmExcel.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        if (dt != null)
                        {
                            for (int i = 0; i < dt.Rows.Count; i++)
                            {
                                ...
                                string cod_sunat_tipo_zona = dt.Rows[i][16].ToString();
                                string cod_sunat_tipo_via = dt.Rows[i][18].ToString();
                                string cod_sunat_departamento = dt.Rows[i][28].ToString();
                                string cod_sunat_provincia = cod_sunat_departamento + dt.Rows[i][29].ToString();
                                string cod_sunat_distrito = cod_sunat_provincia + dt.Rows[i][30].ToString();
                                string cod_sunat_sedeempresa = dt.Rows[i][31].ToString();
                                string cod_area_empresa = cod_sunat_empresa + cod_sunat_sedeempresa;
                                string cod_sunat_area = dt.Rows[i][32].ToString();
                                string cod_cargo_empresa = cod_area_empresa + cod_sunat_area;
                                string cod_sunat_cargo = dt.Rows[i][33].ToString();
                                string cod_sunat_tipotrabajador_salud = dt.Rows[i][34].ToString();
                                ...
                                infogeneralist.Add(new eTrabajadorMasivo()
                                {
                                    ...
                                    cod_estadocivil = lstestadocivil.FirstOrDefault(td => td.cod_sunat == cod_sunat_estadocivil)?.cod_estado_civil,
                                    correo_laboral = dt.Rows[i][11].ToString(),
                                    ...
                                    imp_sueldo_base = dt.Rows[i][43].ToString(),
                                    dsc_porcentajequincena = dt.Rows[i][44].ToString(),
                                    dsc_porcentajecomision = dt.Rows[i][45].ToString(),
                                    ...
                                    dsc_cta_bancaria = dt.Rows[i][50].ToString(),
                                    dsc_cta_interbancaria = dt.Rows[i][51].ToString(),
                                    ...
                                    fch_vencimiento = dt.Rows[i][61].ToString(),
                                    ...
                                    dsc_cta_bancaria_CTS = dt.Rows[i][66].ToString(),
                                    dsc_cta_interbancaria_CTS = dt.Rows[i][67].ToString(),
        ...
        private void gvTrabajador_ValidateRow(object sender, ValidateRowEventArgs e)
        {
            ...
            GridColumn civil = view.Columns["dsc_estadocivil"];
        ...
        private void gvTrabajador_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            for (int i = 0; i < gvTrabajador.RowCount; i++)
            {
                ...
                if (objFact.cod_tipo_documento == null) { total = y + 1; } else { total = y; }
                ...
                foreach (GridColumn col in gvTrabajador.Columns)
                {
                    ...
                    //if (col.FieldName == "dsc_apellido_paterno" || col.FieldName == "dsc_apellido_materno" || col.FieldName == "dsc_nombres")
                    //{
                    //    apellido = objFact.dsc_apellido_paterno; apellidom = objFact.dsc_apellido_materno; nombres = objFact.dsc_nombres;
                    //    objFact.dsc_nombres_completos = apellido + " " + apellidom + " " + nombres;
                    //}
                    ...
                    //if (col.FieldName == "cod_distrito")
                    //{

                    //    var nombre_tipovia = "";
                    //    string cod_tipo_via = Convert.ToString(view.GetFocusedRowCellValue("cod_tipo_via"));
                    //    if (cod_tipo_via == null || cod_tipo_via == "") { nombre_tipovia = ""; } else { nombre_tipovia = OBTENERTIPOVIA(cod_tipo_via); }

                    //    var nmbre_departamento = "";
                    //    string cod_departamento = Convert.ToString(view.GetFocusedRowCellValue("cod_departamento"));
                    //    if (cod_departamento == null || cod_departamento == "") { nmbre_departamento = ""; } else { nmbre_departamento = OBTENERDEP(cod_departamento); }

                    //    var nmbre_provincia = "";
                    //    string cod_provincia = Convert.ToString(view.GetFocusedRowCellValue("cod_provincia"));
                    //    if (cod_provincia == null || cod_provincia == "") { nmbre_provincia = ""; } else { nmbre_provincia = OBTENERPROV(cod_provincia); }

                    //    var nmbre_distrito = "";
                    //    string cod_distrito = Convert.ToString(view.GetFocusedRowCellValue("cod_distrito"));
                    //    if (cod_distrito == null || cod_distrito == "") { nmbre_distrito = ""; } else { nmbre_distrito = OBTENERDIS(cod_distrito); }
                    //}

                    //if (col.FieldName == "cod_sede_empresa")
                    //{
                    //    var sede_empresa = "";
                    //    string cod_sede_empresa = Convert.ToString(view.GetFocusedRowCellValue("cod_sede_empresa"));
                    //    if (cod_sede_empresa == null || cod_sede_empresa == "") { sede_empresa = ""; } else { sede_empresa = obtenersede(cod_sede_empresa); }
